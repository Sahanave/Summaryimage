...
int outImageWidth = 5*352, outImageHeight = 288; // change here to the required output image size
                     ...
                     int       nCmdShow)
{
 ...
 // change here to the required output image size
     inImage.setWidth(w);
	 inImage.setHeight(h);
	if ( strstr(ImagePath, ".rgb" )==NULL )
	{ 
		AfxMessageBox( "Image has to be a '.rgb' file\nUsage - Image.exe image.rgb");
		//return FALSE;
	}
	else
	{
		...
		if (!inImage.ReadImage())
		{
			...
			outImage.setWidth(5*352);
			outImage.setHeight(288);
			
			
			
			outImage.setImageData(inImage.Modify());
			//outImage.WriteImage();
		}
	}
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   hWnd = CreateWindow(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW,
      0, 0,10*outImageWidth, outImageHeight+50, NULL, NULL, hInstance, NULL);
		...
		case WM_PAINT:
			{
				...
				SetDIBitsToDevice(hdc,
								  0,20,outImage.getWidth(),outImage.getHeight(),
